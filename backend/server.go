...
type ServerState struct {
	...
	// faceVerificationClient is nil when face verification is disabled for this
...
func handleFaceCaptureConfig(state *ServerState, w http.ResponseWriter, r *http.Request) {
	...
	// environments.
	//
	// Disabled face verification 404s even with the URL configured, so the
	// capture page and the flag cannot disagree: no liveness session gets
	// captured that issuance will never consume (and that nothing would
	// delete).
	if !state.faceVerificationEnabled() || state.regulaFaceApiPublicUrl == "" {
