...
// TestFaceCaptureConfigNotFoundWhenDisabled verifies disabled face
// verification 404s the capture page even with the public URL still
// configured, so the page and the flag cannot disagree: no liveness session
// gets captured that issuance will never consume.
func TestFaceCaptureConfigNotFoundWhenDisabled(t *testing.T) {
	state := &ServerState{
		// No faceVerificationClient → face verification disabled.
		regulaFaceApiPublicUrl: "https://faceapi.staging.yivi.app",
	}
